...
const SDataTable = ({
  ...
  customActions = [], // e.g., [{ label: 'Action', onClick: (row) => {} }]
  ...
  const processedColumns = React.useMemo(() => {
    const cols = [...columns];
    if ((actions && actions.length > 0) || (customActions && customActions.length > 0)) {
      cols.push({
        key: 'sdt_actions',
        label: 'Actions',
        sortable: false,
          ...
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            ...
            {customActions && customActions.map((action, idx) => (
              <SButton
                ...
                style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0.5rem', background: 'transparent', border: 'none', cursor: 'pointer', ...action.style }}
              />
            ))}
          </div>
        )
  ...
  return (
    <div className={`s-data-table ${disableContainer ? '' : 'page-container'}`} style={{ position: 'relative' }}>
      ...
      {tabs && tabs.length > 0 && (
        <div className="sdt-tabs-container" style={{ margin: 0, padding: '0 1rem', borderBottom: '1px solid var(--border)', display: 'flex', gap: '1rem' }}>
          {tabs.map(tab => (
            <button
              key={tab.id}
              ...
              style={{
                ...
                color: activeTab === tab.id ? 'var(--accent)' : 'var(--text-muted, var(--text-secondary))',
              ...
              onMouseOver={(e) => {
                if (activeTab !== tab.id) {
                  ...
                  e.target.style.borderBottom = '2px solid var(--border-hover, #ccc)';
                }
              }}
              onMouseOut={(e) => {
                if (activeTab !== tab.id) {
                  e.target.style.color = 'var(--text-muted, var(--text-secondary))';
      ...
      <div className="sdt-header-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', gap: '1rem' }}>
        <div className="sdt-title">
          ...
          {subTitle && <span style={{ margin: 0, fontSize: '0.875rem', color: 'var(--text)' }}>{subTitle}</span>}
        </div>
