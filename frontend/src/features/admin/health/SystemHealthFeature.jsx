...
const SystemHealthFeature = () => {
  ...
  const fetchHealth = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.error || t('admin.health.fetch_failed', 'Failed to fetch system health'));
      }
    } catch (err) {
      toast.error(t('admin.health.network_error', 'Network error while fetching system health'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      healthy: { label: t('admin.health.healthy', 'Healthy'), className: 'status-healthy' },
      degraded: { label: t('admin.health.degraded', 'Degraded'), className: 'status-degraded' },
      down: { label: t('admin.health.down', 'Down'), className: 'status-down' }
    };
  ...
  const serviceNames = {
    backendUptime: t('admin.health.backend_api', 'Backend API'),
    supabaseDatabase: t('admin.health.supabase_db', 'Supabase Database'),
    supabaseStorage: t('admin.health.supabase_storage', 'Supabase Storage'),
    redis: t('admin.health.redis_cache', 'Redis Cache'),
    socketIo: t('admin.health.socket_io', 'Socket.IO (Realtime)')
  };
  ...
  return (
    <div className="system-health-container page-container">
      <div className="health-header" style={{ display: 'flex', flexDirection: 'column', alignItems: 'stretch' }}>
        ...
        {healthData && (
          <p className="health-subtitle">
            {t('admin.health.overall_status', 'Overall Status:')} {getStatusBadge(healthData.status)}
            <span className="health-time">
              {t('admin.health.last_checked', 'Last checked:')} {new Date(healthData.timestamp).toLocaleTimeString()}
            </span>
          </p>
        )}
      </div>
      ...
      {loading && !healthData ? (
        <div className="health-loading">{t('admin.health.loading', 'Loading system status...')}</div>
      ) : healthData ? (
        <div className="health-grid">
          
          <div className="health-card">
            <div className="health-card-header">
              <h3 className="health-card-title">{t('admin.health.frontend_spa', 'Frontend SPA')}</h3>
              {getStatusBadge('healthy')}
            </div>
            <div className="health-card-body">
              <div className="health-message">
                {t('admin.health.uptime', 'Uptime:')} {frontendUptime}
              </div>
            </div>
          </div>
                ...
                <div key={key} className="health-card">
                  ...
                  <div className="health-card-body">
                    {service.responseTime !== undefined && (
                      <div className="health-metric">
                        <span className="metric-label">{t('admin.health.ping', 'Ping:')}</span>
                  ...
                  {parsedMessage ? (
                    <div className="health-message" style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '8px' }}>
                      <div className="health-metric">
                        <span className="metric-label">{t('admin.health.storage', 'Storage:')}</span>
                        <span className="metric-value">{parsedMessage.used_mb} MB / 10 GB </span>
                      </div>
                      <div className="health-metric">
                        <span className="metric-label">{t('admin.health.class_a_ops', 'Class A Ops:')}</span>
                        <span className="metric-value">{parsedMessage.class_a.toLocaleString()} / 1M </span>
                      </div>
                      <div className="health-metric">
                        <span className="metric-label">{t('admin.health.class_b_ops', 'Class B Ops:')}</span>
                        <span className="metric-value">{parsedMessage.class_b.toLocaleString()} / 10M </span>
                      </div>
                    </div>
                  ) : service.message ? (
                    <div className="health-message">
                      {key === 'backendUptime' ? `${t('admin.health.uptime', 'Uptime:')} ${service.message}` : service.message}
                    </div>
                  ) : null}
                </div>
              </div>
        ...
        <div className="health-error">{t('admin.health.failed_load', 'Failed to load system health.')}</div>
      )}
    </div>
  );
};
